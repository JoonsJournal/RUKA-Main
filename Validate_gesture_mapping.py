...
class GestureMappingValidator:
    ...
    def interactive_validation(self):
        ...
        ✅ 버그 수정 (2가지):
        1. 타이머 처리 버그: continue 시 timer.end_loop() 누락 → try-finally로 보호
        2. 이미지 표시 버그: img에 그리고 pose_result["img"] 표시 → display_img 사용
        3. 키 입력 안정성: waitKey(1) → waitKey(5)로 증가
        """
        print("\n" + "=" * 70)
        ...
        while not self.quit_flag:
            ...
            try:
                # ✅ 수정 1: try-finally로 타이머 보호
                success, img = self.cap.read()
                if not success:
                    # ✅ 수정: continue 전에 end_loop() 호출 보장
                    continue
                
                pose_result = self.detect_hand_pose(img)
                
                # ✅ 수정 2: pose_result["img"]를 display_img로 복사
                # 이제 display_img에 텍스트를 그리면 화면에 표시됨
                display_img = pose_result["img"].copy()
                ...
                if pose_result["detected"]:
                    for hand in pose_result["hands"]:
                        if hand["type"] == self.hand_type:
                            ...
                            # ✅ 수정: display_img에 텍스트 추가
                            cv2.putText(display_img, status, (10, 30),
                                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                
                # 조작 안내
                cv2.putText(display_img, "SPACE: Capture | r: Robot | o/c: Compare | q: Quit", 
                           (10, display_img.shape[0] - 20),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
                
                # ✅ 수정: display_img 표시
                cv2.imshow("Interactive Validation", display_img)
                
                # ✅ 수정 3: waitKey 시간을 5ms로 증가 (키 입력 안정성 향상)
                ...
                # ✅ 수정: 모든 경우에 timer.end_loop() 호출 보장
                self.timer.end_loop()
