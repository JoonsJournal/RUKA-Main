...
# MediaPipe to Oculus 매핑 (webcam_teleoperator.py에서 가져옴)
MEDIAPIPE_TO_OCULUS_MAPPING = {
    0: 0,    # 손목
    1: 1, 2: 2, 3: 3, 4: 4,      # 엄지
    5: 5, 6: 6, 7: 7, 8: 8,      # 검지
    9: 9, 10: 10, 11: 11, 12: 12,  # 중지
    13: 13, 14: 14, 15: 15, 16: 16,  # 약지
    17: 17, 18: 18, 19: 19, 20: 20,  # 새끼
}
...
class GestureMappingValidator:
    ...
    # =========================================================================
    # 좌표 변환 메서드 (webcam_teleoperator.py에서 가져옴)
    # =========================================================================
    ...
    def _get_hand_dir_frame(self, origin_coord, index_knuckle_coord, pinky_knuckle_coord, hand_name):
        ...
        if hand_name == "left":
            cross_product = normalize_vector(index_knuckle_coord - pinky_knuckle_coord)
        else:
            cross_product = normalize_vector(pinky_knuckle_coord - index_knuckle_coord)
            ...
            while time.time() - start_time < WAIT_SECONDS:
                ...
                if pose_result["detected"]:
                    for hand in pose_result["hands"]:
                        if hand["type"] == self.hand_type:
                            # 좌표 변환 및 모터 값 계산
                            oculus_format = self._mediapipe_to_oculus_format(hand["lmList"])
                            oculus_format = oculus_format / 1000.0
                            transformed_coords, _ = self.transform_keypoints(oculus_format, self.hand_type)
    ...
    def interactive_validation(self):
        ...
        while not self.quit_flag:
            ...
            if key == ord('q'):
                ...
                if current_data and self.robot:
